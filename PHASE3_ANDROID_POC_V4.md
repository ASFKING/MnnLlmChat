...
| # | 任务 | 状态 | 完成日期 | 备注 |
|---|------|------|----------|------|
| 4.1 | 实现 VectorStore（内存向量库） | ✅ | 2026-05-18 | 内存暴力搜索，点积相似度 |
| 4.2 | 实现 RAGEngine 的分块逻辑 | ✅ | 2026-05-18 | 段落分割 + 句号切分 + 重叠 |
| 4.3 | 实现 RAGEngine 的索引 + 检索 + prompt 组装 | ✅ | 2026-05-18 | 完整 RAG 流程 |
| 4.4 | 准备 3-5 篇测试文档 | ✅ | 2026-05-18 | 真机验证用 |
| 4.5 | 实现 RAGFragment UI | ✅ | 2026-05-18 | 文档上传 + 对话问答 |
| 4.6 | 端到端验证 | ✅ | 2026-05-18 | 文档索引 + 基于文档的问答均正常 |

### D.7 Day 5：结构化提取 + 文档生成

| # | 任务 | 状态 | 完成日期 | 备注 |
|---|------|------|----------|------|
| 5.1 | 设计结构化提取 prompt 模板 | ✅ | 2026-05-18 | JsonUtils.buildExtractPrompt() |
| 5.2 | 实现 JsonUtils | ✅ | 2026-05-18 | JSON 提取 + 校验 + prompt 构建 |
| 5.3 | 实现 ChatFragment 中的结构化提取功能 | ✅ | 2026-05-18 | extractStructured() 方法 |
| 5.4 | 准备 100 条测试数据 | ✅ | 2026-05-18 | 真机验证 |
| 5.5 | 测试 100 次，统计正确率 | ✅ | 2026-05-18 | 测试通过，正确率达标 |
| 5.6 | 实现文档生成功能 | ✅ | 2026-05-18 | GenerationStore + generateDocument() + "生成"按钮 |
| 5.7 | 优化 prompt（如正确率不达标） | ⏭️ | — | 正确率已达标，跳过 |

### D.8 Day 6：多模态（图片理解）

| # | 任务 | 状态 | 完成日期 | 备注 |
|---|------|------|----------|------|
| 6.1 | 注册多模态模型到 ModelRegistry | ✅ | 2026-05-18 | 改为 Qwen3.5-2B-MNN（2GB，阿里官方确认支持 Vision） |
...
| 2026-05-18 | VisionFragment.kt 编译报错 Missing `}` | 本地文件被截断，末尾缺少类关闭花括号 `}`。用仓库版本覆盖修复 | 10min |
...
| 2026-05-18 | Day 6 多模态模型改用 Qwen3.5-2B-MNN，弃用 Qwen2.5-VL-3B | 查阿里官方 model_market.json 确认 Qwen3.5 系列 tags=["Think","Vision"]，支持图像理解。2GB 比 Qwen2.5-VL-3B（2.5GB）更小，8GB 设备更友好 |
| 2026-05-18 | VisionFragment 加载 VL 模型前先释放已有的 LLM 模型 | 8GB 设备不能同时驻留两个大模型。释放 Qwen3（~1.5GB）后再加载 Qwen3.5-2B（~2GB），避免 OOM |
